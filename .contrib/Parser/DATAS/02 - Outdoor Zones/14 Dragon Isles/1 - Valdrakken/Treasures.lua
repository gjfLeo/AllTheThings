---------------------------------------------------
--          Z O N E S        M O D U L E         --
---------------------------------------------------
root(ROOTS.Zones, m(DRAGON_ISLES, bubbleDown({ ["timeline"] = { ADDED_10_0_2_LAUNCH } }, {
	m(VALDRAKKEN, {
		n(TREASURES, {
			o(381691, {	-- A Steamy Romance Novel: The Lusty Draconian Mage
				["coord"] = { 47.7, 15.4, VALDRAKKEN },
				["groups"] = { i(200264) },	-- A Steamy Romance Novel: The Lusty Draconian Mage
			}),
			o(381277, {	-- Barrel of Confiscated Treats
				["coord"] = { 9.6, 56.3, VALDRAKKEN },
				["questID"] = 70731,
				["groups"] = {
					i(198106),	-- Recipe: Tasty Hatchling's Treat (RECIPE!)
					i(197769),	-- Tasty Hatchling's Treat
				},
			}),
			o(421718, {	-- Dragon Pepe
				["coord"] = { 50.5, 52.9, VALDRAKKEN },
				["timeline"] = { ADDED_10_2_5 },
				["groups"] = { i(213181) },	-- A Tiny Dragon Goblet (Pepe!)
			}),
		}),
	}),
})));
