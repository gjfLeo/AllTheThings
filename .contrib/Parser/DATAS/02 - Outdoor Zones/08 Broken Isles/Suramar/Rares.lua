---------------------------------------------------
--          Z O N E S        M O D U L E         --
---------------------------------------------------

root(ROOTS.Zones, {
	m(BROKEN_ISLES, {
		m(SURAMAR, {
			n(RARES, {
				n(111649, {	-- Ambassador D'vwinn
					["questID"] = 43794,
					["coord"] = { 54.8, 63.8, SURAMAR },
					["groups"] = {
						i(139918),	-- Robes of the Nightborne Ambassador
					},
				}),
				n(111197, {	-- Anax
					["questID"] = 43954,
					["coord"] = { 33.7, 51.8, SURAMAR },
					["groups"] = {
						i(140934),	-- Benax (PET!)
					},
				}),
				n(110870, {	-- Apothecary Faldren
					["questID"] = 43580,
					["coord"] = { 42.2, 56.4, SURAMAR },
					["groups"] = {
						i(121754),	-- Mana-Flecked Mantle
					},
				}),
				n(110656, {	-- Arcanist Lylandre
					["questID"] = 43481,
					["coord"] = { 65.6, 59.1, SURAMAR },
				}),
				n(106351, {	-- Artificer Lothaire
					["questID"] = 43717,
					["coord"] = { 33.8, 15.1, SURAMAR },
				}),
				n(110726, {	-- Cadraeus
					["questID"] = 43495,
					["coords"] = {
						{ 62.7, 49.1, SURAMAR },
						{ 62.6, 47.1, SURAMAR },
					},
					["groups"] = {
						i(139969),	-- Nightborne Stalker's Coif
					},
				}),
				n(100864, {	-- Cora'kar
					["questID"] = 41135,
					["coord"] = { 68.2, 58.6, SURAMAR },
					["groups"] = {
						i(139952),	-- Salt-Tide Jailor's Tunic
					},
				}),
				n(111651, {	-- Degren
					["questID"] = 43792,
					["coord"] = { 54.4, 56.1, SURAMAR },
					["groups"] = {
						i(121808),	-- Nether Conductors
					},
				}),
				n(99792, {	-- Elfbane
					["questID"] = 41319,
					["coord"] = { 22.1, 51.8, SURAMAR },
					["groups"] = {
						i(121806),	-- Mountain Rage Shaker
					},
				}),
				n(99610, {	-- Garvrulg
					["questID"] = 40897,
					["coord"] = { 53.3, 30.2, SURAMAR },
					["groups"] = {
						i(121755),	-- Slatecrusher Spaulders
					},
				}),
				n(110832, {	-- Gorgroth
					["questID"] = 43992,
					["coord"] = { 27.3, 66.0, SURAMAR },
					["groups"] = {
						i(121747),	-- Soulcrush Legplates
					},
				}),
				n(110944, {	-- Guardian Thor'el
					["questID"] = 43597,
					["coords"] = {
						{ 61.0, 53.0, SURAMAR },
						{ 59.5, 52.0, SURAMAR },
						{ 58.0, 51.1, SURAMAR },
						{ 56.3, 50.3, SURAMAR },
					},
				}),
				n(103214, {	-- Har'kess the Insatiable
					["questID"] = 41136,
					["coord"] = { 67.6, 71.0, SURAMAR },
				}),
				n(103223, {	-- Hertha Grimdottir
					["questID"] = 43993,
					["coord"] = { 61.3, 39.7, SURAMAR },
					["groups"] = {
						i(121737),	-- Exile's Chain Boots
					},
				}),
				n(106532, {	-- Inquisitor Volitix
					["questID"] = 44569,
					["coord"] = { 38.0, 70.4, SURAMAR },
				}),
				n(103827, {	-- King Morgalash
					["questID"] = 41786,
					["coord"] = { 87.3, 62.3, SURAMAR },
				}),
				n(106526, {	-- Lady Rivantas
					["questID"] = 44675,
					["coord"] = { 35.4, 67.0, SURAMAR },
					["groups"] = {
						i(141866),	-- Rivantas' Silk Gloves
					},
				}),
				n(102303, {	-- Lieutenant Strathmar
					["questID"] = 40905,
					["coord"] = { 48.5, 56.7, SURAMAR },
					["groups"] = {
						i(121735),	-- Nightguard Stompers
					},
				}),
				n(113368, {	-- Llorian
					["questID"] = 44676,
					["coord"] = { 29.4, 53.3, SURAMAR },
					["groups"] = {
						i(141867),	-- Llorian's Dress Leggings
					},
				}),
				n(109954, {	-- Magister Phaedris
					["questID"] = 43348,
					["coord"] = { 42.1, 80.1, SURAMAR },
					["groups"] = { i(140405) },	-- Illusion Matrix Crystal
				}),
				n(112497, {	-- Maia the White
					["questID"] = 44072,
					["coord"] = { 24.4, 35.2, SURAMAR },
					["groups"] = {
						i(139897),	-- Cloak of the White Prowler
					},
				}),
				n(110024, {	-- Mal'Dreth the Corruptor
					["questID"] = 43351,
					["coord"] = { 34.2, 61.0, SURAMAR },
				}),
				n(112802, {	-- Mar'tura
					["questID"] = 44124,
					["coord"] = { 13.5, 53.4, SURAMAR },
				}),
				n(111329, {	-- Matron Hagatha
					["questID"] = 43718,
					["coord"] = { 36.2, 33.8, SURAMAR },
				}),
				n(111653, {	-- Miasu
					["questID"] = 43793,
					["coord"] = { 62.5, 63.7, SURAMAR },
					["groups"] = {
						i(121810),	-- Pocket Void Portal
					},
				}),
				n(110340, {	-- Myonix
					["questID"] = 43358,
					["coord"] = { 41.0, 32.7, SURAMAR },
					["groups"] = {
						i(133816),	-- Recipe: Leybeque Ribs [Rank 1] (RECIPE!)
						i(121739),	-- Felinus Manus
					},
				}),
				n(110577, {	-- Oreth the Vile
					["questID"] = 43449,
					["coord"] = { 24.6, 47.4, SURAMAR },
				}),
				n(107846, {	-- Pinchshank
					["questID"] = 43968,
					["coord"] = { 66.7, 67.1, SURAMAR },
					["groups"] = {
						i(140314),	-- Crab Shank (TOY!)
						i(140402),	-- Green Or'ligai Egg
					},
				}),
				n(111007, {	-- Randril
					["questID"] = 43603,
					["coord"] = { 49.6, 79.0, SURAMAR },
				}),
				n(105547, {	-- Rauren
					["questID"] = 43484,
					["coord"] = { 24.0, 25.5, SURAMAR },
					["groups"] = {
						i(121759),	-- Feather-Notched Belt
					},
				}),
				n(103575, {	-- Reef Lord Raj'his
					["questID"] = 44003,
					["coords"] = {
						{ 78.4, 56.4, SURAMAR },
						{ 76.3, 58.6, SURAMAR },
						{ 74.6, 56.9, SURAMAR },
					},
					["groups"] = {
						i(121801),	-- Coral-Edge Band
					},
				}),
				n(103183, {	-- Rok'nash
					["questID"] = 40680,
					["coords"] = {
						{ 80.2, 70.6, SURAMAR },
						{ 80.0, 68.0, SURAMAR },
					},
					["groups"] = {
						i(140019),	-- Eel Collars
					},
				}),
				n(113425, {	-- Rovendros
					["questID"] = 43795,
					["coord"] = { 54.7, 46.1, SURAMAR },
					["description"] = "Found inside |cFFFFD700Teloth'aran|r trapped in a web. Free him and he'll join you in Shal'Aran!",
				}),
				n(103841, {	-- Shadowquill
					["questID"] = 43996,
					["coord"] = { 16.8, 26.6, SURAMAR },
				}),
				n(109054, {	-- Shal'an
					["questID"] = 42831,
					["coord"] = { 26.3, 41.2, SURAMAR },
					["groups"] = {
						i(139926),	-- Subject 12's ID Bracelets
					},
				}),
				n(110438, {	-- Siegemaster Aedrin
					["questID"] = 43369,
					["coord"] = { 37.3, 21.7, SURAMAR },
				}),
				n(110824, {	-- Tideclaw
					["questID"] = 43542,
					["coord"] = { 18.7, 61.1, SURAMAR },
				}),
				n(99899, {	-- Vicious Whale Shark
					["questID"] = 44669,
					["coords"] = {
						{ 78.3, 64.8, SURAMAR },
						{ 80.0, 64.9, SURAMAR },
						{ 78.7, 61.1, SURAMAR },
						{ 78.1, 57.3, SURAMAR },
						{ 75.9, 58.8, SURAMAR },
					},
				}),
			}),
		}),
	}),
});
