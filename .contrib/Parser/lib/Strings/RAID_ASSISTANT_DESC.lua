createLocalizationString({
	readable = "Never enter the instance with the wrong settings again! Verify that everything is as it should be!",
	constant = "RAID_ASSISTANT_DESC",
	export = true,
	text = {
		en = "Never enter the instance with the wrong settings again! Verify that everything is as it should be!",
		de = "Betreten Sie die Instanz nie wieder mit den falschen Einstellungen! Überprüfen Sie, ob alles so ist, wie es sein sollte!",
		es = "Nunca entres a una instancia con los ajustes incorrectos! Verifica que todo está como debería!",
		-- TODO: mx = "",
		-- TODO: fr = "",
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Больше никогда не заходите в подземелье с неправильными настройками! Проверяйте, всё ли так, как нужно!",
		-- TODO: ko = "",
		cn = "再也不要用错误的设置进入团队了！检查一切是否正常！",
		tw = "再也不要用錯誤的設定進入團隊了！檢查一切是否正常！",
	},
})