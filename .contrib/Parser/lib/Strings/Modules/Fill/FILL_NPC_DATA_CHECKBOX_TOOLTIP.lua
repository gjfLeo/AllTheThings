createLocalizationString({
	readable = "Enable this option if you want to fill all relevant data for a given NPC (Common Boss Drops, Drops, etc). This option may cause a significant amount of duplication, but the idea is that the NPC will remain visible if you need something available from that NPC.\n\nNote: A lot of Dragonflight outdoors content relies on this setting being enabled for accuracy due to how many Rares share common drops.\n\nDefault: Off",
	constant = "FILL_NPC_DATA_CHECKBOX_TOOLTIP",
	export = true,
	text = {
		...
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Включите данную опцию, если Вы хотите видеть все вложенные данные для существ (Общая добыча, Добыча, и т.п.) отображаемых в мини-списке. Эта опция может привести к значительному дублированию Штучек, но идея в том, чтоб существа оставались в мини-списке, как будто с них что-то ещё нужно.\n\nПо умолчанию: отключено",
		-- TODO: ko = "",
		cn = "如果希望在迷你列表中显示时将所有相关数据嵌套到给定的 NPC（常见首领掉落、掉落等），请启用此选项。此选项可能会导致大量重复，但其想法是该 NPC 将在迷你列表中保持可见，就好像需要特定于该 NPC 的内容一样。\n\n默认：关闭",
		tw = "如果希望在迷你列表中顯示時將所有相關資料嵌套到給定的 NPC（常見首領掉落、掉落等），請啟用此選項。此選項可能會導致大量重複，但其想法是該 NPC 將在迷你列表中保持可見，就好像需要特定於該 NPC 的內容一樣。\n\n預設：關閉",
	},
})