createLocalizationString({
	readable = "Use this to customize your desired level of precision in percentage calculations.\n\nDefault: 2",
	constant = "PRECISION_SLIDER_TOOLTIP",
	export = true,
	text = {
		en = "Use this to customize your desired level of precision in percentage calculations.\n\nDefault: 2",
		-- TODO: de = "",
		es = "Usa esto para personalizar el nivel de precisión deseado en los cálculos de porcentajes.\n\nPor defecto: 2",
		-- TODO: mx = "",
		fr = "Utilisez cette option pour personnaliser le niveau de précision souhaité dans les calculs de pourcentage.\n\nDéfaut : 2",
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Используйте для установления желаемой точности для процентных рассчётов.\n\nПо умолчанию: 2",
		-- TODO: ko = "",
		cn = "使用此选项可自定义百分比计算中所需的精度级别。\n\n默认：2",
		tw = "使用此選項可自訂百分比計算中所需的精度級別。\n\n預設：2",
	},
})