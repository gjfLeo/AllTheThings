createLocalizationString({
	readable = "Generate Dynamic Categories based only on the very highest Category.",
	constant = "DYNAMIC_CATEGORY_SIMPLE_TOOLTIP",
	export = true,
	text = {
		en = "Generate Dynamic Categories based only on the very highest Category.",
		-- TODO: de = "",
		es = "Genera categorías dinámicas basándose sólo en la categoría más alta.",
		-- TODO: mx = "",
		fr = "Générer des catégories dynamiques en se basant uniquement sur la catégorie la plus haute",
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Генерировать Динамические Категории только на основе самой общей Категории.",
		-- TODO: ko = "",
		cn = "仅根据最高的类别生成动态类别。 ",
		tw = "僅依據最高的類別生成動態類別。 ",
	},
})