createLocalizationString({
	readable = "Enable this option to show the estimated amount of Items/Currency required to collect Things.\n\nFor Containers which do not reward all of their available content at once, the estimate will thus be lower than actually required.",
	constant = "SHOW_CURRENCY_CALCULATIONS_CHECKBOX_TOOLTIP",
	export = true,
	text = {
		...
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Включите данную опцию, если Вы хотите видеть в подсказке приблизительное количество Предметов/Валюты, необходимое для покупки Штучек.\n\nДля Контейнеров, которые не гарантируют получение всего содержимого за раз, оценка затрат будет ниже.",
		-- TODO: ko = "",
		cn = "启用此选项以显示收集物品所需的物品/货币的估计数量。\n\n对于不一次奖励所有可用内容的容器，估计将因此低于实际需要。",
		tw = "啟用此選項以顯示收集物品所需的物品/貨幣的估計數量。\n\n對於不一次獎勵所有可用內容的容器，估計將因此低於實際需要。",
	},
})