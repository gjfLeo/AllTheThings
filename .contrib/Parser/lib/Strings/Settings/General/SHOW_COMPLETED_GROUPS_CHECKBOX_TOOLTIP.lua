createLocalizationString({
	readable = "Enable this option if you want to see completed groups as a header with a completion percentage. If a group has nothing relevant for your class, this setting will also make those groups appear in the listing.\n\nWe recommend you turn this setting off as it will conserve the space in the mini list and allow you to quickly see what you are missing from the zone.",
	constant = "SHOW_COMPLETED_GROUPS_CHECKBOX_TOOLTIP",
	export = true,
	text = {
		...
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Включите данную опцию, если Вы хотите видеть выполненные группы как заголовок с процентом выполнения. Если группа не имеет ничего важного для Вашего класса, эта настройка также отобразит такие группы в списке.\n\nМы рекомендуем Вам отключить эту настройку, поскольку будет занимать место в Мини списке и мешать быстро определить, что доступно в текущей зоне.",
		-- TODO: ko = "",
		cn = "如果想在标题中看到已完成的组和完成百分比，请启用此选项。如果一个组没有与你职业相关的内容，这个设置也会让这些组出现在列表中。\n\n我们建议你关闭此设置，因为它将节省小列表中的空间，并允许你快速查看区域中缺少的内容。",
		tw = "如果想在標題中看到已完成的組和完成百分比，請啟用此選項。如果一個組沒有與你職業相關的內容，這個設定也會讓這些組出現在列表中。\n\n我們建議你關閉此設定，因為它將節省小列表中的空間，並允許你快速查看區域中缺少的內容。",
	},
})