createLocalizationString({
	readable = "Enable this setting if you want to show content which requires Pet Battles within the game.",
	constant = "SHOW_PET_BATTLES_CHECKBOX_TOOLTIP",
	export = true,
	text = {
		en = "Enable this setting if you want to show content which requires Pet Battles within the game.",
		-- TODO: de = "",
		es = "Activa este ajuste si quieres ver el contenido que requiere Batallas de mascotas en el juego.",
		-- TODO: mx = "",
		fr = "Activez cete option si vous souhaitez afficher du contenu nécessitant des combats de mascottes dans le jeu.",
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Включите данную опцию, если Вы хотите отслеживать весь контент, который требует Битвы Питомцев.",
		-- TODO: ko = "",
		cn = "如果您想在游戏中显示需要宠物对战的内容，请启用此设置。",
		tw = "如果您想在遊戲中顯示需要寵物對戰的內容，請啟用此設定。",
	},
})