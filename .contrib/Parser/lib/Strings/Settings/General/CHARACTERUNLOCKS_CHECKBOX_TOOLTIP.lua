createLocalizationString({
	readable = "Enable this option to track Character Unlocks. These are various character-based unlocks which aren't clearly able to be categorized as another type (e.g. Hex variants, Polymorph variants, Hunter species taming unlocks, Pocopoc customizations, etc.)\n\nTracked per character by default.",
	constant = "CHARACTERUNLOCKS_CHECKBOX_TOOLTIP",
	export = true,
	text = {
		...
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Включите для отслеживания Разблокирований Персонажа. Это различные разблокирования, которые не могут быть отнесены к другим типам (например, варианты Сглазов и Превращений, разблокирования питомцев для Охотников, кастомизации Покопока и прочее).\n\nПо умолчанию отслеживается на каждом персонаже отдельно.",
		-- TODO: ko = "",
		cn = "启用此选项可跟踪角色解锁。这些是基于角色的各种解锁，无法明确归类为其他类型（例如 妖术变体、变形术变体、猎人物种驯服解锁、波可波克自定义等）。\n\n默认情况下按角色跟踪。",
		tw = "啟用此選項可追蹤角色解鎖。這些是基於角色的各種解鎖，無法明確歸類為其他類型（例如 妖術變體、變形術變體、獵人物種馴服解鎖、波可波克自訂等）。\n\n預設情況下按角色追蹤。",
	},
})