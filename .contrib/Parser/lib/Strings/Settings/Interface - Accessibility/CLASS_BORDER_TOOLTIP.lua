createLocalizationString({
	readable = "Use your class color for the borders. This updates when you log onto another class.",
	constant = "CLASS_BORDER_TOOLTIP",
	export = true,
	text = {
		en = "Use your class color for the borders. This updates when you log onto another class.",
		-- TODO: de = "",
		es = "Usa el color de tu clase para los bordes. Se actualiza cuando entras al juego con otra clase.",
		-- TODO: mx = "",
		fr = "Utilise la couleur de votre classe pour les bords. Cette couleur est mise à jour lorsque vous vous connectez sur une autre classe.",
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Использовать цвет вашего класса для рамки. Цвет также обновляется, когда Вы на другом классе.",
		-- TODO: ko = "",
		cn = "使用职业颜色作为边框。当登录到另一个职业时，此信息会更新。",
		tw = "使用職業顏色作為邊框。當登錄到另一個職業時，此資訊會更新。",
	},
})