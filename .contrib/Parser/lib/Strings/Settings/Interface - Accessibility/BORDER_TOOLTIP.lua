createLocalizationString({
	readable = "Set the border color of all ATT windows.",
	constant = "BORDER_TOOLTIP",
	export = true,
	text = {
		en = "Set the border color of all ATT windows.",
		-- TODO: de = "",
		es = "Define el color del borde de todas las ventanas de ATT.",
		-- TODO: mx = "",
		fr = "Définit la couleur des bords de toutes les fenêtres d’ATT.",
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Установка цвета рамки для всех окон ATT.",
		-- TODO: ko = "",
		cn = "设置所有 ATT 窗口的边框颜色和透明度。",
		tw = "設定所有 ATT 視窗的邊框顏色和透明度。",
	},
})