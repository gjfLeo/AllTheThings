createLocalizationString({
	readable = "This may be unable to be completed without Party Sync if completing any of these quests first:",
	constant = "BREADCRUMB_PARTYSYNC",
	export = true,
	text = {
		en = "This may be unable to be completed without Party Sync if completing any of these quests first:",
		de = "Dies ist nicht abschließbar ohne Partysyncronisation, wenn etwas aus folgendem abgeschlossen ist:",
		es = "Es posible que esto no se pueda completar sin Sincronización de grupo si primero se completa alguna de estas misiones:",
		-- TODO: mx = "",
		-- TODO: fr = "",
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Может быть недоступно для выполнения без Синхронизации группы, если выполнить любое из этих заданий:",
		-- TODO: ko = "",
		cn = "如果先完成这些任务中的任何一个在没有小队同步的情况下可能无法完成：",
		tw = "如果先完成這些任務中的任何一個在沒有隊伍同步的情況下可能無法完成：",
	},
})