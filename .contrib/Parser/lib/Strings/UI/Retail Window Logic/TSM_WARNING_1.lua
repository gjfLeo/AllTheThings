createLocalizationString({
	readable = "Running this command can potentially destroy your existing TSM settings by reassigning items to the ",
	constant = "TSM_WARNING_1",
	export = true,
	text = {
		en = "Running this command can potentially destroy your existing TSM settings by reassigning items to the ",
		de = "Diesen Befehl auszuführen kann möglicherweise Ihre bestehenden TSM Einstellungen zerstören, indem es sich in die ",
		es = "Ejecutar este comando puede destruir potencialmente su configuración TSM existente al reasignar elementos a la ",
		-- TODO: mx = "",
		-- TODO: fr = "",
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Использование данной команды может сломать Ваши существующие настройки TSM при перераспределении предметов к ",
		-- TODO: ko = "",
		cn = "运行此命令可能会通过将物品重新分配到 ",
		tw = "執行此命令可能會通過將物品重新分配到 ",
	},
})