createLocalizationString({
	readable = "All recipes that you have not collected yet are displayed here.",
	constant = "ALL_THE_RECIPES_DESC",
	export = true,
	text = {
		en = "All recipes that you have not collected yet are displayed here.",
		de = "Alle Rezepte die Ihr noch nicht gesammelt habt, werden hier angezeigt.",
		es = "Todas las recetas que aún no has coleccionado se muestran aquí.",
		-- TODO: mx = "",
		-- TODO: fr = "",
		-- TODO: it = "",
		-- TODO: pt = "",
		ru = "Все рецепты, что Вы ещё не получили, отображаются здесь.",
		-- TODO: ko = "",
		cn = "所有你尚未收藏的食谱都会显示在这里。",
		tw = "所有你尚未收藏的食譜都會顯示在這裡。",
	},
})